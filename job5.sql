...
-- 4. L'Allemagne (80 millions d'habitants) est le pays le plus peuplé d'Europe. L'Autriche (8,5 millions d'habitants) compte 11% de la population allemande. Créez une requête permettant d’afficher le nom et la population de chaque pays d'Europe, en pourcentage de la population de l'Allemagne.
SELECT name, CONCAT(ROUND(population/(SELECT population FROM world WHERE name = 'Germany')*100, 2), '%') AS 'population' FROM world WHERE continent = 'Europe';
...
*SELECT continent FROM world WHERE 25000000 >= ALL(SELECT population FROM world WHERE continent = world.continent);